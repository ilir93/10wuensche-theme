...
function category_page_save_meta_box_data($post_id) {
    ...
    if (isset($_POST['category_image_type']) && $_POST['category_image_type'] == 'upload') {
        ...
        $old_h1_title = get_post_meta($post_id, '_category_h1_title_previous', true);
        $new_h1_title = get_post_meta($post_id, '_category_h1_title', true) ?: get_the_title($post_id);
        ...
        if ($old_h1_title !== $new_h1_title) {
            ...
            if (isset($_POST['category_image_data']) && !empty($_POST['category_image_data'])) {
                ...
                $filename = sanitize_file_name($new_h1_title) . '-category.png';
                ...
                if (!$existing_image_id) {
                    ...
                    $attachment = array(
                        ...
                        'post_title' => $new_h1_title . '-category',
                        'post_content' => '',
                        'post_status' => 'inherit'
                    );
                    ...
                    // Use existing image
                    update_post_meta($post_id, '_category_image', $existing_image_id);
            ...
            update_post_meta($post_id, '_category_h1_title_previous', $new_h1_title);
        }
        // If H1 hasn't changed, keep the existing image (don't update _category_image meta)
    }
